...
// v3.2 — “All-in-One”: Family+ACL+MAR+Barcode + Minor-Ack + Livestream stubs + Policy/Telemetry
//         + Geofencing + Devices (IP Agent) + Parental Enforce (restart/shutdown/app) + Files (Local Presign)
//
// HOW TO RUN (dev):
...
// Optional deps (safe to miss in dev)
const webpush: any = (() => { try { return require('web-push'); } catch { return null; } })();
const twilio: any = (() => { try { return require('twilio'); } catch { return null; } })();
...
interface TaskItem extends Shareable {
  ...
  ackAt?: number
  ackBy?: string
  __minorStage?: MinorStage
  __lastMinorPingAt?: number
}
...
/* Devices with either IP agent or generic metadata */
...
;[uOwner, uCare, uFam, uChild].forEach((u) => {
  db.users.set(u.id, u)
...
app.post('/api/tasks', auth, (req: AuthedRequest, res) => {
  ...
  audit(req.user!.id, 'create', 'task', tsk.id, { repeat: tsk.repeat })
...
app.post('/api/tasks/:id/ack', auth, (req: AuthedRequest, res) => {
  ...
  t.ackAt = Date.now()
  t.ackBy = req.user!.id
  t.__minorStage = asStage(Math.max(t.__minorStage ?? 0, 2))
...
/** Parental remote enforcement (signal-only) */
app.post('/api/parental/enforce', auth, (req: AuthedRequest, res) => {
  ...
  fanout(
    ...
    { targetUserId: target.id, action, reason: reason || '' },
  )
...
app.get('/api/devices', auth, (req: AuthedRequest, res) => {
  ...
  const list = Array.from(db.devices.values()).filter(
    (d) =>
      d.circleId === cid &&
      (!userId || d.userId === userId || d.ownerUserId === userId),
  )
  return res.json(list)
})
app.post('/api/devices', auth, (req: AuthedRequest, res) => {
  ...
  audit(req.user!.id, 'create', 'device', dev.id, {
    name: dev.name,
    kind: dev.kind,
    ip: dev.ip,
    port: dev.port,
  })
  fanout(cid, 'device_registered', `Device added: ${dev.name}`, {
    deviceId: dev.id,
  })
  res.json(dev)
})
app.delete('/api/devices/:id', auth, (req: AuthedRequest, res) => {
  ...
  try {
    ensureCircleAccess(req.user!, dev.circleId)
  } catch {
    return res.status(403).json({ error: 'forbidden' })
  }
  db.devices.delete(dev.id)
  audit(req.user!.id, 'delete', 'device', dev.id)
  fanout(dev.circleId, 'device_removed', `Device removed: ${dev.name}`, {
    deviceId: dev.id,
  })
  return res.json({ ok: true })
})
...
app.post('/api/devices/:id/ping', auth, async (req: AuthedRequest, res) => {
  ...
  try {
    ensureCircleAccess(req.user!, d.circleId)
  } catch {
    return res.status(403).json({ error: 'forbidden' })
  }
...
setInterval(() => {
  ...
  // Vitals threshold checks (recent)
