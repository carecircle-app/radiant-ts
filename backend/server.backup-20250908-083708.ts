 ...
 * (Fix: TS2367 in minor reminder flow by using local stage state machine.)
 *
 * HOW TO RUN (dev):
...
app.post('/api/tasks/:id/ack', auth, (req: AuthedRequest, res) => {
  ...
  tsk.ackAt = Date.now()
  tsk.ackBy = req.user!.id
  if (req.body?.photoKey) tsk.proofKey = String(req.body.photoKey)
  db.tasks.set(tsk.id, tsk)
  audit(req.user!.id, 'ack', 'task', tsk.id, { proof: !!req.body?.photoKey })
  return res.json({ ok: true })
})
...
// PUT raw bytes to a nested key, using a RegExp route (compatible with strict path-to-regexp)
...
setInterval(() => {
  ...
  // Tasks: general due reminders + escalation
  for (const tsk of db.tasks.values()) {
    if (tsk.completed) continue

    // Minor protocol: T-15 heads-up → +5m (10m left) → +5m (5m left) → +5m disruptive, until ack/complete
    if (tsk.forMinor && tsk.due && tsk.ackRequired && !tsk.ackAt) {
      const delta = tsk.due - nowTs
      const pingGap = 5 * 60_000
      if (delta <= 15 * 60_000) {
        let stage = tsk.__minorStage ?? 0
        const last = tsk.__lastMinorPingAt ?? 0
        let updated = false

        if (stage === 0) {
          // first time in 15m window
          stage = 1
          updated = true
          fanout(
            tsk.circleId,
            'minor_headsup',
            `Heads-up: ${tsk.title} in 15 minutes`,
          )
        } else if (stage === 1 && nowTs - last >= pingGap) {
          stage = 2
          updated = true
          fanout(
            tsk.circleId,
            'minor_ping',
            `Reminder: ${tsk.title} soon (10m left)`,
          )
        } else if (stage === 2 && nowTs - last >= pingGap) {
          stage = 3
          updated = true
          fanout(
            tsk.circleId,
            'minor_ping',
            `Reminder: ${tsk.title} soon (5m left)`,
          )
        } else if (stage === 3 && nowTs - last >= pingGap) {
          stage = 4
          updated = true
          fanout(
            tsk.circleId,
            'disruptive_alert',
            `Action needed: ${tsk.title}`,
          )
        }

        if (updated) {
          tsk.__minorStage = stage
  ...
  // Vitals threshold checks (recent 15m)
