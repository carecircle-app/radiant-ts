...
if (Test-Path $server) { Copy-Item $server "$server.backup-$ts" }
if (Test-Path $admin)  { Copy-Item $admin  "$admin.backup-$ts"  } else { New-Item -ItemType Directory -Force .\app\admin | Out-Null }

# === Load server.ts ===
$content = Get-Content -Raw $server

# 1) Insert Device types before the DB header
$deviceTypes = @'
/* --- Devices model (registry for IP-controlled agents) --- */
...
interface Device {
  ...
  agentPath?: string; // default "/control"
  agentSecret?: string;
  lastSeenAt?: number;
}
'@

if ($content -notmatch 'interface\s+Device\s*\{') {
  $content = $content -replace '(?ms)(\n/\* ---------------------------- DB \(in-memory\) ---------------------------- \*/)', "`n$deviceTypes`n`$1"
}

# 2) Add devices: Map to DB object
if ($content -notmatch 'devices:\s*new Map<string,\s*Device>') {
  $content = $content -replace "telemetry:\s*new Map<string,\s*TelemetryEvent>\(\),", "telemetry: new Map<string, TelemetryEvent>(),`r`n  devices: new Map<string, Device>(),"
}

# 3) Insert Devices Registry endpoints before the Livestream section
$devicesBlock = @'
/* ---------------------------- Devices Registry ---------------------------- */

/**
 * Optional local agent contract (you run this on the child's PC/phone):
 * POST http://<ip>:<port>/control
 *   { "action": "device_restart"|"device_shutdown"|"app_restart"|"ping", "reason":"...", "secret":"<shared>" }
 * Returns: { ok: true }
 *
 * Minimal Windows agent example (PowerShell, run as admin):
 ...
 *      if ($body.secret -ne "SHARED") { $ctx.Response.StatusCode = 403 }
 *      else {
 *        switch ($body.action) {
 *          "device_restart" { shutdown /r /t 0 }
 *          "device_shutdown" { shutdown /s /t 0 }
 *          "app_restart" { # your app restart logic }
 *          "ping" { }
 *        }
...
app.get('/api/devices', auth, (req: AuthedRequest, res) => {
  ...
  const list = Array.from(db.devices.values()).filter(d => d.circleId === cid);
  res.json(list);
});

app.post('/api/devices', auth, (req: AuthedRequest, res) => {
  const cid = req.user!.circleId || seedCircle.id;
  const { name, kind, ip, port, agentPath, agentSecret, ownerUserId } = req.body || {};
  ...
  const dev: Device = {
    ...
    port: port != null ? Number(port) : 8088,
    agentPath: agentPath ? String(agentPath) : '/control',
    agentSecret: agentSecret ? String(agentSecret) : undefined,
    lastSeenAt: Date.now()
  };
  db.devices.set(dev.id, dev);
  audit(req.user!.id, 'create', 'device', dev.id, { name: dev.name, kind: dev.kind, ip: dev.ip, port: dev.port });
  res.json(dev);
});

async function tryNotifyAgent(d: Device, action: string, reason?: string) {
  try {
    if (!(globalThis as any).fetch) return { ok: false, status: 0, error: 'fetch_unavailable' };
  } catch { return { ok: false, status: 0, error: 'fetch_unavailable' }; }
  ...
  try {
    const r = await (fetch as any)(url, {
      ...
      body: JSON.stringify({ action, reason: reason || '', secret: d.agentSecret || '' }),
      signal: ctrl.signal
    });
    clearTimeout(t);
...
app.post('/api/devices/:id/ping', auth, async (req: AuthedRequest, res) => {
  const d = db.devices.get(req.params.id);
  if (!d) return res.status(404).json({ error: 'not found' });
  try { ensureCircleAccess(req.user!, d.circleId); } catch { return res.status(403).json({ error: 'forbidden' }); }
  const out = await tryNotifyAgent(d, 'ping', 'connectivity-test');
  if (out.ok) { d.lastSeenAt = Date.now(); db.devices.set(d.id, d); }
  res.json(out);
});
'@

if ($content -notmatch 'Devices Registry') {
  $content = $content -replace '(?ms)(\n/\* ---------------------------- Livestream \(stubs\) ---------------------------- \*/)', "`n$devicesBlock`n`$1"
}

# 4) Replace /api/devices/:id/intent with an agent-calling version
$patternIntent = "app\.post\('/api/devices/:id/intent'[\s\S]*?\)\;"
$intentNew = @'
app.post('/api/devices/:id/intent', auth, async (req: AuthedRequest, res) => {
  ...
  if (d) {
    const out = await tryNotifyAgent(d, String(action), String(reason || ''));
    return res.json({ ok: true, agent: out });
  }
  return res.json({ ok: true });
});
'@

if ($content -match $patternIntent) {
  $content = [regex]::Replace($content, $patternIntent, $intentNew, 'Singleline')
}

# 5) Expand allowed parental actions to include restart/shutdown/app_restart
$patternAllowed = "const\s+allowed\s*=\s*new\s+Set\(\[[^\]]*\]\);"
$replaceAllowed = "const allowed = new Set(['screen_lock','network_pause','play_loud_alert','restrict_apps','device_restart','device_shutdown','app_restart']);"
if ($content -match "app\.post\('/api/parental/enforce'") {
  $content = [regex]::Replace($content, $patternAllowed, $replaceAllowed, 'Singleline')
}

# === Save server.ts ===
Set-Content -Path $server -Value $content -Encoding UTF8

# === Write Admin page (clean replacement with Device Registry) ===
$adminContent = @'
"use client";
...
export default function AdminPage() {
  ...
  async function createMinorTasks() {
    ...
    try {
      ...
      alert(`Created ${selectedChildIds.length} task(s) ✅`);
      loadAll();
    } catch (e: any) {
...
Write-Host "✅ Patches written. Now restart both terminals:"
Write-Host "Terminal A (API):"
Write-Host '$env:PORT="4000"; $env:DEV_ALLOW_HEADER="1"; $env:ALLOWED_ORIGINS="http://127.0.0.1:3000,http://localhost:3000"; $env:FILES_DIR=".\uploads"; npx tsx backend/server.ts'
Write-Host "Terminal B (WEB):"
Write-Host 'npx next dev -H 127.0.0.1 -p 3000'
