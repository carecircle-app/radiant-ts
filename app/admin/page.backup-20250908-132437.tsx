...
export default function AdminPage() {
  ...
  async function loadAll() {
    ...
    try {
      ...
      // Initialize default assignee list if empty
      if (assignees.length === 0) {
        const initial = u.filter(x => x.role === "Child").slice(0, 1).map(x => x.id);
        if (initial.length) setAssignees(initial);
      }
    } catch (err) {
      console.error(err);
  ...
  function toggleAssignee(id: string) {
    setAssignees(prev => {
      ...
      if (prev.length >= MAX_CHILDREN) return prev; // enforce cap
      return [...prev, id];
    });
  }
  ...
  async function createMinorTask() {
    ...
    const targets = (assignees.length ? assignees : childUsers.slice(0,1).map(c=>c.id)).slice(0, MAX_CHILDREN);
    if (targets.length === 0) { alert("No child selected"); return; }

    try {
      await Promise.all(targets.map(a => {
        const body = { title, due, assignedTo: a, forMinor: true, ackRequired, photoProof };
      ...
      alert(`Created ${targets.length} task(s) `);
      await loadAll();
    } catch (e) {
    ...
    <div className="min-h-dvh p-6 space-y-8">
      ...
      <section className="grid gap-2 border rounded p-4">
        <div className="flex items-center justify-between">
          ...
          <div className="text-xs text-gray-500">Select up to {MAX_CHILDREN} children</div>
        </div>
        ...
        <div className="mt-2 grid gap-2">
          ...
          <div className="flex flex-wrap gap-2">
            {childUsers.map(c => {
              ...
              const maxed = assignees.length >= MAX_CHILDREN && !checked;
              return (
                <label key={c.id} className={cls("flex items-center gap-2 border rounded px-2 py-1", maxed && "opacity-50")}>
                  <input
                    type="checkbox"
                    checked={checked}
                    disabled={maxed}
            ...
            {childUsers.length === 0 && <div className="text-xs text-gray-500">No Child users found.</div>}
          </div>
          <div className="text-xs text-gray-500">Selected: {assignees.length}/{MAX_CHILDREN}</div>
        </div>

        <div>
          <button className="border rounded px-3 py-1" onClick={createMinorTask} disabled={loading || (assignees.length === 0 && childUsers.length === 0)}>
            Create {assignees.length > 0 ? assignees.length : 1} task{(assignees.length > 1) ? "s" : ""}
          </button>
        </div>
      </section>
