...
/* ===== Robust mapper: PLAN_FEATURES item -> /admin#anchor ===== */
function normalize(text: string) {
  return text
    .replace(/[^\p{L}\p{N}\s]/gu, " ") // strip emoji/punct
    .toLowerCase()
    .replace(/\s+/g, " ")
    .trim();
}

const ANCHOR_RULES: Array<{ re: RegExp; id: string }> = [
  { re: /\b(kid|children?)\b/, id: "kids" },
  { re: /\bcalendar\b/, id: "calendar" },
  { re: /\b(chore|reminder)s?\b/, id: "chores" },
  { re: /\bchat\b/, id: "chat" },
  { re: /\b(geofenc|arrival|leave)\b/, id: "geofencing" },
  { re: /\b(notification|alert|push)\b/, id: "notifications" },
  { re: /\b(network|lock|pause|shut\s*down|shutdown)\b/, id: "network" },
  { re: /\bvideo\b/, id: "video" },
  { re: /\b(shopping|receipt|receipts|budget|budgets)\b/, id: "shopping" },
  { re: /\bpark(ing)?\b/, id: "parking" },
  { re: /\bhome\b.*\bschool\b/, id: "home-school" },
  { re: /\bsos\b|\bfall\b/, id: "sos" },
  { re: /\b(vital|bp|heart|o2|oxygen|temp|temperature|weight)\b/, id: "vitals" },
  { re: /\b(diabetes|insulin|sugar)\b/, id: "diabetes" },
  { re: /\b(medicine|mar)\b/, id: "medicine" },
];

function toFromFeature(item: any): { to: string | null; label: string } {
  // explicit override if present in PLAN_FEATURES
  if (item && typeof item.to === "string") {
    return { to: item.to, label: item.text ?? item.label ?? item.to };
  }
  const label =
    typeof item === "string" ? item : typeof item?.text === "string" ? item.text : "";
  if (!label) return { to: null, label: "" };
  const n = normalize(label);
  for (const { re, id } of ANCHOR_RULES) {
    if (re.test(n)) return { to: `/admin#${id}`, label };
  }
  return { to: null, label };
}

function FeatureAuto({ item }: { item: any }) {
  const { to, label } = toFromFeature(item);
  return to ? (
    <li>
      <Link
        href={to}
        className="inline-flex items-center gap-2 hover:underline rounded focus:outline-none focus:ring-2 focus:ring-emerald-500"
      >
        {label}
      </Link>
    </li>
      ...
      <main id="main">
        ...
        <section className="relative bg-gradient-to-b from-emerald-50 via-sky-50 to-white">
          <div className="mx-auto max-w-6xl px-6 py-20 text-center">
            ...
            {/* Small plan CTAs (no tall side bars) */}
        ...
        {/* ===== Pricing (drawer cards; buttons stay small) ===== */}
                ...
                <div className="mt-6 grid gap-6 md:grid-cols-3 text-left">
                  ...
                  <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                    ...
                    <ul className="mt-4 space-y-2 text-sm text-slate-700">
                      {PLAN_FEATURES.free.map((t: any, i: number) => (
                        <FeatureAuto key={i} item={t} />
                      ))}
                    </ul>
                    <a
                      href="/pricing"
                      className="mt-5 inline-flex w-full justify-center rounded-md border border-slate-300 px-3 py-2 text-sm hover:bg-slate-50"
                    >
                      See full details
                    </a>
                  </div>
                  ...
                  <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                    ...
                    <ul className="mt-4 space-y-2 text-sm text-slate-700">
                      {PLAN_FEATURES.lite.map((t: any, i: number) => (
                        <FeatureAuto key={i} item={t} />
                      ))}
                    </ul>
                    <a
                  ...
                  <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                    ...
                    <ul className="mt-4 space-y-2 text-sm text-slate-700">
                      {PLAN_FEATURES.elite.map((t: any, i: number) => (
                        <FeatureAuto key={i} item={t} />
                      ))}
                    </ul>
                    <a
        ...
        <section id="foundation" className="mx-auto max-w-6xl px-6 pb-24">
          <div className="text-center">
            ...
            <p className="mt-2 text-slate-600">
              Every subscription helps. From each <strong>Elite plan</strong>, we donate <strong>$1.00</strong>. From
              each <strong>Lite plan</strong>, we donate <strong>$0.50</strong>. 100% of dedicated donations go to the
              Foundation.
            </p>
            <div className="mt-6 flex flex-col sm:flex-row justify-center gap-3">
              <a
                ...
                className={
                  "inline-flex items-center rounded-full px-4 py-2 text-sm font-medium text-white shadow-sm transition " +
                  "bg-emerald-600 hover:bg-emerald-700 focus:outline-none focus:ring-2 focus:ring-emerald-500/50 " +
                  (!env.donateOnce ? "pointer-events-none opacity-50" : "")
                }
              >
              ...
              <a
                href={
                  env.donateMonthly
                    ? "/api/stripe/checkout?priceId=" + env.donateMonthly + "&mode=subscription"
                    : "#"
                }
                aria-disabled={!env.donateMonthly}
                className={
                  "inline-flex items-center rounded-full px-4 py-2 text-sm font-medium text-white shadow-sm transition " +
                  "bg-amber-500 hover:bg-amber-600 focus:outline-none focus:ring-2 focus:ring-amber-500/50 " +
                  (!env.donateMonthly ? "pointer-events-none opacity-50" : "")
                }
              >
                Donate $1.50 / month
              </a>
