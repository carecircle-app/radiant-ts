...
interface Device {
  ...
  userId: string;
  name: string;
  kind: "phone" | "computer" | "tablet" | "other";
  controlUrl?: string;
  platform?: string;
  lastSeenAt?: number;
}
...
app.get('/api/devices', auth, (req: AuthedRequest, res) => {
  ...
  const list = Array.from(db.devices.values()).filter(d =>
    d.circleId === cid && (!userId || d.userId === userId)
  );
  return res.json(list);
});

app.post('/api/devices', auth, (req: AuthedRequest, res) => {
  const { userId, name, kind, controlUrl, platform } = req.body || {};
  ...
  try { ensureCircleAccess(req.user!, user.circleId || seedCircle.id); } catch { return res.status(403).json({ error: 'forbidden' }); }
  const dev: Device = {
    id: id('dev'),
    circleId: user.circleId || seedCircle.id,
    userId: user.id,
    name: String(name),
    kind: String(kind) as any,
    controlUrl: controlUrl ? String(controlUrl) : undefined,
    platform: platform ? String(platform) : undefined,
    lastSeenAt: Date.now(),
  };
  db.devices.set(dev.id, dev);
  audit(req.user!.id, 'create', 'device', dev.id, { userId: user.id, kind: dev.kind });
  fanout(dev.circleId, 'device_registered', `Device added: ${dev.name}`, { deviceId: dev.id });
  return res.json(dev);
});

app.delete('/api/devices/:id', auth, (req: AuthedRequest, res) => {
  const dev = db.devices.get(req.params.id);
  if (!dev) return res.status(404).json({ error: 'not found' });
  try { ensureCircleAccess(req.user!, dev.circleId); } catch { return res.status(403).json({ error: 'forbidden' }); }
  db.devices.delete(dev.id);
  audit(req.user!.id, 'delete', 'device', dev.id);
  fanout(dev.circleId, 'device_removed', `Device removed: ${dev.name}`, { deviceId: dev.id });
  return res.json({ ok: true });
});
